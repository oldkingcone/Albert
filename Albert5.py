...
try:
    ...
    import nmap
    import csv
    from sys import urlib3
    from os import path
    from subprocess import call
    from api import apikey
    from tqdm import tqdm as tqdm
    from sys import smtplib
    import time
    import base64
    import os
...
def nmapScan(tgtHost, tgtPort,arguemts="-sX -T2 -Pn -O -f --traceroute --reason --script firewall-bypass -oX):  # Nmap function created
    ...
    while run == 't':
        try:
            ...
            if options == '2':
                ...
                try:
                    nmapScan(host, port, arguments=args)
            ...
            if options == "5":
                ...
                continue
            if options == "6":
                print("[ * ] Hey! Hey! Hey! Sorry, that is a coming feature! [ * ]")
                continue
            if options == "7":
                file = open('vulnersapi.py', 'w')
                SHODAN_API_KEY = input('[*] Hey! Hey! Hey! Enter A Valid VulnersCom API Key: ')
                oppsie = ["apikey= ", "\"", str(VULNERS_API_KEY), "\""]
                file.write(''.join(oppsie))                                                      ##   You must edit this section in order for the scan to work
                print('[~] File Dropped Nigga: ./vulnersapi.py')                                   ##  <OS> = Add Operating System Detected
                file.close()                                                                          ##
                print('[~] Take 5 To Larp Around\n {}'.format(logo))                                     ##
                return False
                def vulners():
                    list = str(input("\n\n\n\t[ + ] Please Choose Option:\n" 
                                        "\t\t1. Operating System Input\n"
                ...
                if options == "1":
                    os.system('cls')
                    choice = str(input("[ + ] Please Input the OS: \n->"))

                    continue
                if options == "2":
                    os.system('cls')
                    choice1 = str(input("[ + ] Please Input The Os Version: \n"))

                if options == "3":
                    os.system('cls')
                    choice2 = str(input("[ + ] Please Input The OS Package \n"))

                    while run == 't':
                        try:
                            os.system('cls')
                            all_cve = vulners_api.archive("cve")
                text_ai_score = vulners_api.aiScore(" Flamming Botnet"                                            ##
                print('[~] Hey! Hey! Hey! Time To Put Your BigBoy Pants On, Self Audit!')
                {choice}_vulnerabilities = vulners_api.audit(os= '{choice} ', os_version= '{choice1} ', package= ['{choice2} '])
                vulnerable_packages = "{choice}"_vulnerabilities.get('pacakge')
                missed_patches_ids = "{choice}"_vulnerabilities.get('vulnerabilitites')
                cve_list = "{choice}"_vulnerabilities.get('cvelist')
                def csv():
                    csvfile = "/albert/Output"
                continue
            if options == "8"
                os.system('cls')
                def __start__():
                    ...
                    print(" ex:0.0.0.0")
                    server = raw_input("SMPT > server >>> ")
                    print(" Please Enter Port")
                    port = input("SMPT > port >>> ")
                    print(" Enter Username File")
                    user = raw_input("SMTP > user >>> ")
                    print("Enter Password File")
                    passwd = raw_input("SMTP > pass >> ")
                    try:
                        userOpen = open(user , "r")
                        userWord = userOpen.readlines()
                        userOpen.close()
                    except IOError:
                        print("[-]No User file found: "+user+end
                              sys.exit()
                    try:
                        passOpen = open(passwd, "r")
                        passWORD = passOPEN.readline()
                        passOPEN.close()
                    except IOERROR:
            ...
            if options == ('9'):
                os.system('cls')
                        def iplocator():
                            print("[+] Please Enter Domain or IP")
                                    victim = raw_input('ip locator > victim >>> '+end)
