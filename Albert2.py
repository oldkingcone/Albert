...
if __name__ == '__main__':
    ...
    # @todo, Crypto Enumeration (SFTP, POPS, TLS, SSL, SSH, SMTPs
    def agreement():
        print("This tools is used for Education Purposes only. We will not be held liable for adversary usage of this tool")
    run = 't'
    albert_faces()
    sleep(0.4)
    while run == 't':
    ...
    for username in userWORD:
        ...
        try:
            ...
            if options == '10':
                ...
                adminList = ['admin/', 'admin.php/', 'joomla/administartor', 'wp-admin/', 'jm/administrator', 'central/admin/',
                             'install/', 'vb/install', 'cp.php', 'admin_login.php', 'admin/admin-login.php', 'adm_auth.php',
                             'moderator.asp', 'controlpanel.asp', 'user.asp', 'admin_panel/', 'wp-login.cgi', 'admin2.cgi', 'adminpanel.cgi',
                             'nsw/admin/login.cgi', 'webadmin.js', 'webadmin.js', 'login.js', 'acceso,js', 'pages/admin/admin-login.js', 'login.cfm'
                             'user.cfm', ]
            clearSrc():
            for site in self.sites:
                for admin in adminList:
                    try:
                        if urlib.urlopen(site + admin).getcode() == 200:
                            print "[-] Found Da Panel -> ", site + admin
                            except IOError
                    continue
            if options == '11':
