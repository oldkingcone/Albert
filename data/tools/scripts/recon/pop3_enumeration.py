...
class POP3UserEnumeration():
    ...
    if __name__ == "__main__":
        ...
        try:
            if (int(args.port) < 1) or (
                    int(args.port) > 65536):  # convert args.port to integer and make sure it fits port range
        ...
        print "[*] %s scan chosen for use against %s:%s" % (
        scantype.upper(), args.target, str(args.port))  # give info on scan back to user
