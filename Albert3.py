    ...
    while run == 't':
        try:
            ...
            if options == "7":
                ...
                <OS>_vulnerabilities = vulners_api.audit(os= ' ', os_version= ' ', package= [' '])
                vulnerable_packages = <OS>_vulnerabilities.get('pacakge')
                missed_patches_ids = <OS>_vulnerabilities.get('vulnerabilitites')
                cve_list = <OS>_vulnerabilities.get('cvelist')
                continue
            if options == '':
                os.system('cls')
